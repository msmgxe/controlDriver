...
export function quitarArrastre(
  ...
  /* Hasta dónde llega el arrastre en la lista de pedidos: la posición del
     último pedido que es, con seguridad, de una ruta descartada. */
  let ultimoDeArrastre = -1;
  ordenes.forEach((o, i) => {
    if (o.ruta !== null && numerosDescartados.has(o.ruta)) ultimoDeArrastre = i;
  });
  ...
  ordenes.forEach((o, i) => {
    const deRutaAnterior =
      (o.ruta !== null && numerosDescartados.has(o.ruta)) ||
      (o.ruta === null && i < ultimoDeArrastre);
    const fechaPrevia = codigosEnOtrasFechas[o.codigo];

    if (deRutaAnterior) descartados.push({ ...o, motivo: "ruta-anterior" });
    else if (fechaPrevia && fechaPrevia !== jornada.fecha) {
      descartados.push({ ...o, motivo: "ya-guardado", fechaPrevia });
    } else quedan.push(o);
  });
